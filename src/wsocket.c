...
{
	// Shared UTF-8 message buffer returned by this function.
	// Not thread safe, but neither is gia_strerror() either.
	static char sUtf8StringBuffer[GAI_STRERROR_BUFFER_SIZE + 1];

	// Fetch the erorr message in UTF-16 form.
	WCHAR* utf16Message = gai_strerrorW(errorCode);
	sUtf8StringBuffer[0] = '\0';
	if (utf16Message && (utf16Message[0] != L'\0'))
	{
		int conversionLength = WideCharToMultiByte(CP_UTF8, 0, utf16Message, -1, NULL, 0, NULL, NULL);
		if (conversionLength > 0)
		{
			if (conversionLength <= (GAI_STRERROR_BUFFER_SIZE + 1))
			{
				// Convert the UTF-16 string directly to the static UTF-8 buffer.
				WideCharToMultiByte(CP_UTF8, 0, utf16Message, -1, sUtf8StringBuffer, conversionLength, NULL, NULL);
			}
			else
			{
				// Static UTF-8 buffer is too small. Convert the UTF-16 string to a temporary UTF-8 buffer.
				// and then copy that string to the static UTF-8 buffer, truncated to its max length.
				char* utf8Message = (char*)malloc((size_t)conversionLength * sizeof(char));
				if (utf8Message)
				{
					utf8Message[0] = '\0';
					WideCharToMultiByte(CP_UTF8, 0, utf16Message, -1, utf8Message, conversionLength, NULL, NULL);
					memcpy(sUtf8StringBuffer, utf8Message, GAI_STRERROR_BUFFER_SIZE);
					sUtf8StringBuffer[GAI_STRERROR_BUFFER_SIZE] = '\0';
					free(utf8Message);
				}
			}
		}
	...
	// Return the requested error code's message in UTF-8 form.
	return sUtf8StringBuffer;
}
