...
static int create(lua_State *L)
{
  ...
  if (!ctx->context) {
    lua_pushnil(L);
    lua_pushfstring(L, "error creating context: %s (%s)",
	  str_method,
...
static int load_locations(lua_State *L)
{
  ...
  if (SSL_CTX_load_verify_locations(ctx, cafile, capath) != 1) {
    lua_pushboolean(L, 0);
    lua_pushfstring(L, "error loading CA locations: %s, %s (%s)",
	  cafile, capath,
      ERR_reason_error_string(ERR_get_error()));
    return 2;
  }
...
static int load_cert(lua_State *L)
{
  ...
  if (SSL_CTX_use_certificate_chain_file(ctx, filename) != 1) {
    lua_pushboolean(L, 0);
    lua_pushfstring(L, "error loading certificate: filename (%s)",
	  filename,
      ERR_reason_error_string(ERR_get_error()));
    return 2;
  }
...
static int load_key(lua_State *L)
{
  ...
  case LUA_TNIL: 
    if (SSL_CTX_use_PrivateKey_file(ctx, filename, SSL_FILETYPE_PEM) == 1)
      ...
      ret = 2;
      lua_pushboolean(L, 0);
      lua_pushfstring(L, "error loading private key: %s (%s)",
		filename,
        ERR_reason_error_string(ERR_get_error()));
    }
...
static int set_cipher(lua_State *L)
{
  ...
  if (SSL_CTX_set_cipher_list(ctx, list) != 1) {
    lua_pushboolean(L, 0);
    lua_pushfstring(L, "error setting cipher list: %s (%s)",
	  list,
      ERR_reason_error_string(ERR_get_error()));
    return 2;
  }
...
static int set_curve(lua_State *L)
{
  ...
  if (!ret) {
    lua_pushboolean(L, 0);
    lua_pushfstring(L, "error setting elliptic curve: %s (%s)",
	  str,
      ERR_reason_error_string(ERR_get_error()));
    return 2;
  }
...
LSEC_API int luaopen_ssl_context(lua_State *L)
{
  ...
  // IMPORTANT: When the second parameter is NULL, it DOESN'T create an
  // object, nor does it add any object to the Lua stack. It's the call
  // to "luaL_newmetatable()" above that does that.
  luaL_register(L, NULL, meta);

  // IMPORTANT: This creates an object and adds it to the Lua stack.
  luaL_register(L, "plugin_luasec_ssl.context", funcs);
